...
class Dinov3Dataset(BaseDataset):
    ...
    def _init_paths(self):
        ...
        self.dir_feature = os.path.join(self.config["dataset"]["dataset_position"][0],"2D","feature","dinov3",self.config["phase"] + "A")
        self.feature_paths = make_datset_plural([[self.dir_feature]],self.make_dataset, self.config)[0]
        if (len(self.A_paths) != len(self.B_paths)) or (len(self.A_paths) != len(self.Mask_paths)) or (len(self.A_paths) != len(self.feature_paths)):
            print("len A:", len(self.A_paths))
            print("len B:", len(self.B_paths))
            print("len Mask:", len(self.Mask_paths))
            print("len feature:", len(self.feature_paths))
            raise ValueError("The number of images in A, B, Mask, and feature must be the same.")
    ...
    def __getitem__(self, index):
        ...
        return {'A': A, 'B': B, 'Mask': Mask, "feature": feature}
